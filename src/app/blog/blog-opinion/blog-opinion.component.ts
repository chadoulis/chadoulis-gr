...
@Component({
  selector: 'app-blog-opinion',
  templateUrl: './blog-opinion.component.html',
  styleUrls: ['./blog-opinion.component.scss']
})
export class BlogOpinionComponent implements OnInit {
  ...
  filter: any;
    ...
    pageChange(newPage: number) {
      this.router.navigate(['blog/opinion'], { queryParams: { page: newPage } });
    }
     ...
     */
    ngOnInit(): void {
      this.blogService.getBlogPostsOfCategory('opinion').subscribe(res => {
        this.news = res.map( e => {
          return e.payload.doc.data()
        })
        this.news = this.news.slice().sort((a, b) => b.date - a.date)
